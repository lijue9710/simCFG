...
def construct_cost_functions(G1, G2,
    ...
    C_node[0:num_G1_nodes, num_G2_nodes:num_G2_nodes + num_G1_nodes] = np.array([G1_node_deletion_cost[i] if i == j \
                                                                                 ...
                                                                                 for j in range(num_G1_nodes)]).reshape(
    ...
    C_node[num_G1_nodes:num_G1_nodes + num_G2_nodes, 0:num_G2_nodes] = np.array([G2_node_insertion_cost[i] if i == j \
                                                                                 ...
                                                                                 for j in range(num_G2_nodes)]).reshape(
    ...
    C_edge[0:num_G1_edges, num_G2_edges:num_G2_edges + num_G1_edges] = np.array([G1_edge_deletion_cost[i] if i == j \
                                                                                 ...
                                                                                 for j in range(num_G1_edges)]).reshape(
    ...
    C_edge[num_G1_edges:num_G1_edges + num_G2_edges, 0:num_G2_edges] = np.array([G2_edge_insertion_cost[i] if i == j \
                                                                                 ...
                                                                                 for j in range(num_G2_edges)]).reshape(
...
def get_edges_to_match(G, node_id, matched_nodes):
    ...
    if G.has_edges_between(node_id, node_id):
...
class search_tree_node:
    def __init__(self, G1, G2, parent_matched_cost, parent_matched_nodes, parent_matched_edges, node_G1, node_G2, \
                 parent_unprocessed_nodes_G1, parent_unprocessed_nodes_G2, parent_unprocessed_edges_G1,
        ...
        if len(incident_edges_G1) > 0 and len(incident_edges_G2) > 0:  # Consider substituting
            matched_edges_cost_matrix = subset_cost_matrix(cost_matrix_edges, incident_edges_G1, incident_edges_G2,
        ...
        if len(self.unprocessed_nodes_G1) > 0 and len(self.unprocessed_nodes_G2) > 0:  # Consider substituting
            unmatched_nodes_cost_matrix = subset_cost_matrix(cost_matrix_nodes, self.unprocessed_nodes_G1,
                                                             self.unprocessed_nodes_G2, G1.number_of_nodes(),
                                                             G2.number_of_nodes())
            # Match the edges as per the LAP solution
        ...
        if len(self.unprocessed_edges_G1) > 0 and len(self.unprocessed_edges_G2) > 0:  # Consider substituting
            unmatched_edges_cost_matrix = subset_cost_matrix(cost_matrix_edges, self.unprocessed_edges_G1,
                                                             self.unprocessed_edges_G2, G1.number_of_edges(),
                                                             G2.number_of_edges())
            # Match the edges as per the LAP solution
...
def edit_cost_from_node_matching(G1, G2, cost_matrix_nodes, cost_matrix_edges, node_matching):
    ...
    for i in range(len(matched_nodes[0])):
        ...
        incident_edges_G1 = [];
        if matched_nodes[0][
            i] is not None:  # Find the edges with one end-point as node_G1 and other in matched nodes or node_G1
            incident_edges_G1, index_G1, direction_G1 = get_edges_to_match(G1, matched_nodes[0][i],
                                                                           matched_nodes[0][:i])

        incident_edges_G2 = np.array([]);
        if matched_nodes[1][
            i] is not None:  # Find the edges with one end-point as node_G2 and other in matched nodes or node_G2
            incident_edges_G2, index_G2, direction_G2 = get_edges_to_match(G2, matched_nodes[1][i],
                                                                           matched_nodes[1][:i])
...
def contextual_cost_matrix_construction(G1, G2,
    ...
    cost_upper_bound = 2 * (
            node_substitution_cost.sum() + G1_node_deletion_cost.sum() + G2_node_insertion_cost.sum() + 1)
    ...
    cost_matrix[0:num_G1_nodes, num_G2_nodes:num_G2_nodes + num_G1_nodes] = np.array([G1_node_deletion_cost[i] if i == j \
                                                                                      ...
                                                                                      for i in range(num_G1_nodes) \
                                                                                      for j in
                                                                                      range(num_G1_nodes)]).reshape(
        num_G1_nodes, num_G1_nodes);
    cost_matrix[num_G1_nodes:num_G1_nodes + num_G2_nodes, 0:num_G2_nodes] = np.array(
        [G2_node_insertion_cost[i] if i == j \
             else cost_upper_bound \
         for i in range(num_G2_nodes) \
    ...
    selected_deletion_G1 = [
        G1_edge_deletion_cost[np.concatenate((self_edge_list_G1[i], incoming_edges_G1[i], outgoing_edges_G1[i]))] for i
        in range(G1.number_of_nodes())];
    selected_insertion_G2 = [
        G2_edge_insertion_cost[np.concatenate((self_edge_list_G2[i], incoming_edges_G2[i], outgoing_edges_G2[i]))] for i
        in range(G2.number_of_nodes())];
    ...
    for i in range(num_G1_nodes):
        for j in range(num_G2_nodes):
            ...
            temp_edge_cost_matrix[:len(self_edge_list_G1[i]), :len(self_edge_list_G2[j])] = edge_substitution_cost[
                                                                                            self_edge_list_G1[i], :][:,
                                                                                            self_edge_list_G2[j]];
            temp_edge_cost_matrix[len(self_edge_list_G1[i]):len(self_edge_list_G1[i]) + len(incoming_edges_G1[i]),
            len(self_edge_list_G2[j]):len(self_edge_list_G2[j]) + len(incoming_edges_G2[j])] = edge_substitution_cost[
                                                                                               incoming_edges_G1[i], :][
                                                                                               :, incoming_edges_G2[j]];
            temp_edge_cost_matrix[len(self_edge_list_G1[i]) + len(incoming_edges_G1[i]):m,
            len(self_edge_list_G2[j]) + len(incoming_edges_G2[j]):n] = edge_substitution_cost[outgoing_edges_G1[i], :][
                                                                       :, outgoing_edges_G2[j]];
...
def hausdorff_matching(G1, G2,
    ...
    selected_insertion_incoming_G2 = [G2_edge_insertion_cost[incoming_edges_G2[i]] for i in
                                      range(G2.number_of_nodes())];

    selected_deletion_outgoing_G1 = [G1_edge_deletion_cost[outgoing_edges_G1[i]] for i in range(G1.number_of_nodes())];
    selected_insertion_outgoing_G2 = [G2_edge_insertion_cost[outgoing_edges_G2[i]] for i in
                                      range(G2.number_of_nodes())];
    ...
    for i in range(num_G1_nodes):
        for j in range(num_G2_nodes):
            ...
            if len(selected_deletion_G1[i]) > len(selected_insertion_G2[j]):
                idx = np.argpartition(selected_deletion_G1[i],
                                      (len(selected_deletion_G1[i]) - len(selected_insertion_G2[j])));
                edge_hausdorff_lower_bound = selected_deletion_G1[i][
                    idx[:(len(selected_deletion_G1[i]) - len(selected_insertion_G2[j]))]].sum();
            elif len(selected_deletion_G1[i]) < len(selected_insertion_G2[j]):
                idx = np.argpartition(selected_insertion_G2[j],
                                      (len(selected_insertion_G2[j]) - len(selected_deletion_G1[i])));
                edge_hausdorff_lower_bound = selected_insertion_G2[j][
                    idx[:(len(selected_insertion_G2[j]) - len(selected_deletion_G1[i]))]].sum();
...
def a_star_search(G1, G2, cost_matrix_nodes, cost_matrix_edges, max_beam_size):
    ...
    for i in range(len(unprocessed_nodes_G2)):
        tree_node = search_tree_node(G1, G2, matched_cost, matched_nodes, matched_edges, unprocessed_nodes_G1[0],
                                     unprocessed_nodes_G2[i], \
                                     unprocessed_nodes_G1, unprocessed_nodes_G2, unprocessed_edges_G1,
    ...
    while len(open_list) > 0:
        ...
        if len(unprocessed_nodes_G1) == 0 and len(unprocessed_nodes_G2) == 0:
            ...
            for i in range(len(unprocessed_nodes_G2)):
                tree_node = search_tree_node(G1, G2, matched_cost, matched_nodes, matched_edges,
                                             unprocessed_nodes_G1[0], unprocessed_nodes_G2[i], \
                                             unprocessed_nodes_G1, unprocessed_nodes_G2, unprocessed_edges_G1,
                                             unprocessed_edges_G2, \
                                             cost_matrix_nodes, cost_matrix_edges);
                # Insert into open-list, implemented as a heap
            ...
            for i in range(len(unprocessed_nodes_G2)):
                tree_node = search_tree_node(G1, G2, matched_cost, matched_nodes, matched_edges, None,
                                             unprocessed_nodes_G2[i], \
                                             unprocessed_nodes_G1, unprocessed_nodes_G2, unprocessed_edges_G1,
                                             unprocessed_edges_G2, \
                                             cost_matrix_nodes, cost_matrix_edges);
...
def graph_edit_distance(G1, G2,
    ...
    if algorithm == "astar" or algorithm == "beam":
        (matched_cost, matched_nodes, matched_edges) = a_star_search(G1, G2, \
                                                                     cost_matrix_nodes, cost_matrix_edges,
                                                                     max_beam_size);
        return (matched_cost, get_sorted_mapping(matched_nodes, G1.number_of_nodes(), G2.number_of_nodes()),
                get_sorted_mapping(matched_edges, G1.number_of_edges(), G2.number_of_edges()));
